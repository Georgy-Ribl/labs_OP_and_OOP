...
static int ensureCapacity(DynamicArray* a)
{
    if (a->size < a->capacity) return OK;
    size_t newCap = a->capacity ? a->capacity * 2 : 1;
    void* p = realloc(a->data, newCap * a->elementSize);
    if (!p) return ERR_MEM;
    a->data = p;
    a->capacity = newCap;
    return OK;
...
{
    a->data = NULL; a->size = 0; a->capacity = 0; a->elementSize = es;
    return ensureCapacity(a);
}

int daPushBack(DynamicArray* a,const void* e)
{
    int st = ensureCapacity(a);
    if (st!=OK) return st;
...
{
    free(a->data);
    a->data = NULL; a->size = a->capacity = a->elementSize = 0;
}
